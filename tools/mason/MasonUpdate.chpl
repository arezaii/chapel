...
proc parseChplVersion(brick: borrowed Toml?): (programVersion, programVersion) {
  use Regex;
...
proc checkChplVersion(chplVersion, low, high) throws {
  ...
  const formatMessage = "\n\n" +
    ...
    proc parseString(ver:string): programVersion throws {
      var ret : programVersion;
    ...
    if (versions.size == 1) {
      hi = new programVersion(max(int), max(int), max(int));
    } else {
...
proc verifyChapelVersion(brick:borrowed Toml) {
  ...
  const current = new programVersion(tupInfo.major, tupInfo.minor, tupInfo.update);
